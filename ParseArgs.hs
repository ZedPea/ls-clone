...
ls = LS { 
    ...
    almost_all = def &= help "Do not list implied . and ..",
    ...
    list_by_columns = def &= name "C" &= help "list entries by columns",
    color = def &= typ "WHEN" &= help "colorize the output; WHEN can be 'always' (default if omitted), 'auto', or 'never'; more info below",
    ...
    no_sort = def &= name "f" &= help "do not sort, enable -aU, disable -ls --color",
    classify = def &= name "F" &= help "append indicator (one of */=>@|) to entries",
    filetype = def &= name "file-type" &= help "likewise, except to not append '*'",
    ...
    long_no_owner = def &= name "g" &= help "like -l, but do not list owner",
    ...
    human_readable = def &= name "human-readable" &= help "with -l and/or -s, print human readable sizes (e.g., 1K 234M 2G)",
    siUnits = def &= name "si" &= help "likewise, but use powers of 1000 not 1024",
    ...
    inode = def &= help "print the index number of each file",
    ...
    long = def &= name "l" &= help "use a long listing format",
    ...
    comma_separated = def &= name "m" &= help "fill width with a comma separated list of entries",
    num_UID_GID = def &= explicit &= name "numeric-uid-gid" &= help "like -l, but list numeric user and group IDs",
    literal = def &= name "N" &= name "literal" &= help "print raw entry names (don't treat e.g. control characters specially)",
    long_no_group = def &= name "o" &= help "like -l, but do not list group information",
    ...
    reverse' = def &= help "reverse order while sorting",
    recursive = def &= name "R" &= name "recursive" &= help "list subdirectories recursively",
    size = def &= help "print the allocated size of each file, in blocks",
    sort_by_size = def &= name "s" &= help "sort by file size, largest first",
    ...
    mod_time = def &= name "t" &= help "sort by modification time, newest first",
    tab_size = def &= explicit &= name "T" &= name "tabsize" &= typ "COLS" &= help "assume tab stops at each COLS instead of 8",
    no_sort_dir_order = def &= name "U" &= help "do not sort; list entries in directory order",
    sort_by_filename_versions = def &= name "v" &= help "natural sort of (version) numbers withing text",
    width = def &= typ "COLS" &= help "set ouput width to COLS. 0 means no limit",
    lines' = def &= name "x" &= help "list entries by lines instead of by columns",
    sort_by_extension = def &= name "X" &= help "sort alphabetically by entry extension",
    security_context = def &= explicit &= name "Z" &= name "context" &= help "print any security context of each file",
    one_file_per_line = def &= name "1" &= help "list one file per line. Avoid '\\n' with -q or -b"
} &= help "List information about the FILEs (the current directory by default).\nSort entries alphabetically if none of -cftuvSUX nor --sort is specified.\nMandatory arguments to long options are mandatory for short options too."
  &= summary ""
  &= details ["The SIZE argument is an integer and optional unit (example: 10K is 10*1024).","Units are K,M,G,T,P,E,Z,Y (powers of 1024) or KB,MB,...(powers of 1000).","","Using color to distinguish file types is disabled both by default and with --color=never. With --color=auto, ls emits color codes only when standard output is conneted to a terminal. The LS_COLORS environment variable can change the settings. Use the dircolors command to set it.","","Exit status:","0 if OK,","1 is minor problems (e.g., cannot access subdirectory),","2 is serious trouble (e.g., cannot access command-line argument)."]
